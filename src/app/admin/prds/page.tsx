                        ...
                        {prds.map((prd) => (
                            ...
                            >
                                ...
                                <div
                                    ...
                                    onClick={() => setExpandedPrdId(expandedPrdId === prd._id ? null : prd._id)}
                                >
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center space-x-4">
                                            <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center">
                                                <span className="text-sm font-bold text-primary">
                                                    {prd.userId?.name?.charAt(0).toUpperCase() || "?"}
                                                </span>
                                            </div>
                                            <div>
                                                <p className="font-medium text-gray-900 dark:text-white">
                                                    {prd.userId?.name || "Unknown User"}
                                                </p>
                                        ...
                                        <div className="flex items-center space-x-3">
                                            <span className={`px-3 py-1 rounded-full text-xs font-medium ${getStatusBadge(prd.status)}`}>
                                                {prd.status.charAt(0).toUpperCase() + prd.status.slice(1)}
                                            </span>
                                            <span className="text-xs text-gray-500 dark:text-gray-400">
                                                {new Date(prd.submittedAt).toLocaleDateString()}
                                            </span>
                                            <svg
                                                className={`w-5 h-5 text-gray-400 transition-transform ${expandedPrdId === prd._id ? "rotate-180" : ""}`}
                                                fill="none"
                                                stroke="currentColor"
                                                viewBox="0 0 24 24"
                                            >
                                                ...
                                                <div className="mb-4 p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                                                    ...
                                                    <div className="flex items-center justify-between mt-2">
                                                        ...
                                                        <div className="flex space-x-2">
                                                            ...
                                                            <button
                                                                onClick={() => handleAddNote(prd._id)}
                                                                disabled={actionLoading === `note-${prd._id}` || !newNoteContent[prd._id]?.trim()}
                                                                className="px-3 py-1.5 text-sm bg-primary text-white rounded-lg hover:bg-primary-hover disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                                                            >
                                                ...
                                                <ul className="space-y-2">
                                                    {prd.adminNotes.map((note) => (
                                                        <li key={note.id} className="flex items-start space-x-3 p-2 bg-gray-50 dark:bg-gray-700/50 rounded-lg group">
                                                            ...
                                                            <button
                                                                onClick={() => handleDeleteNote(prd._id, note.id)}
