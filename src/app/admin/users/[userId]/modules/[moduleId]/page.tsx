               ...
               <div className="bg-card border border-border p-6 rounded-xl shadow-sm mb-8 flex items-center justify-between">
                  <div>
                     ...
                     <p className="text-muted-foreground text-sm">
                        Current Status: <span className="font-semibold uppercase text-primary">{moduleData.status}</span>
                     </p>
                  </div>
                  ...
                  <div className="space-y-6">
                     ...
                     <div className="space-y-4">
                        {moduleData.tasks && moduleData.tasks.length > 0 ? (
                                 ...
                                 <div key={task._id} className="bg-card border border-border p-6 rounded-xl shadow-sm">
                                    <div className="flex justify-between items-start mb-4">
                                       <div>
                                          ...
                                          <span className="text-sm text-muted-foreground">Added: {new Date(task.createdAt).toLocaleString()}</span>
                                       </div>
                                       <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase ${task.status === "approved" ? "bg-green-100 text-green-700" : task.status === "rejected" ? "bg-red-100 text-red-700" : "bg-yellow-100 text-yellow-700"}`}>{task.status}</span>
                                    </div>
                                    ...
                                    <div className="flex flex-wrap gap-2 mt-4 pt-4 border-t border-border">
                                       {activeCommentId === task._id ? (
                                          <div className="w-full space-y-2">
                                             <textarea value={commentText} onChange={(e) => setCommentText(e.target.value)} className="w-full p-2 border rounded bg-background text-foreground text-sm" placeholder="Add a comment..." rows={2} />
                                             <div className="flex gap-2 justify-end">
                                                <button onClick={() => setActiveCommentId(null)} className="px-3 py-1 text-sm text-muted-foreground hover:text-foreground">
                                                   Cancel
                                                </button>
                                                <button
                                          ...
                                          <>
                                             <button onClick={() => handleTaskAction(task._id, "approved")} className="px-4 py-2 bg-green-600 text-white rounded text-sm hover:bg-green-700 transition">
                                                Approve
                                             </button>
                                             <button onClick={() => handleTaskAction(task._id, "rejected")} className="px-4 py-2 bg-red-600 text-white rounded text-sm hover:bg-red-700 transition">
                                                Reject
                                             </button>
                                             <button
                                                onClick={() => {
                                                   setActiveCommentId(task._id);
                                                   setCommentText(task.adminComment || "");
                                                }}
                           ...
                           <div className="p-8 text-center border-2 border-dashed border-muted rounded-xl text-muted-foreground">No tasks submitted by user yet.</div>
                        )}
                     </div>
                  </div>
                        ...
                        <form onSubmit={handleAddStandup} className="space-y-4">
                           <div className="grid grid-cols-2 gap-4">
                              <div>
                                 ...
                                 <input type="date" value={standupDate} onChange={(e) => setStandupDate(e.target.value)} className="w-full p-2 border rounded bg-background" required />
                              </div>
                              <div>
                                 <label className="block text-sm font-medium mb-1">Status</label>
                                 <select value={standupStatus} onChange={(e) => setStandupStatus(e.target.value as "complete" | "incomplete")} className="w-full p-2 border rounded bg-background">
                           ...
                           <div className="flex items-center gap-2">
                              <input type="checkbox" id="attended" checked={standupAttended} onChange={(e) => setStandupAttended(e.target.checked)} className="w-4 h-4 text-primary rounded focus:ring-primary" />
                           ...
                           <div>
                              ...
                              <textarea value={standupReview} onChange={(e) => setStandupReview(e.target.value)} className="w-full p-2 border rounded bg-background h-24" placeholder="Enter meeting notes or feedback..." />
                           </div>

                           <button type="submit" disabled={submittingStandup} className="w-full py-2 bg-primary text-primary-foreground rounded font-medium hover:bg-primary/90 disabled:opacity-50">
                              ...
                              .map((standup) => (
                                 <div key={standup._id} className="bg-card/50 border border-border p-4 rounded-lg">
                                    <div className="flex justify-between items-start mb-2">
                                       <span className="font-medium text-foreground">{new Date(standup.date).toLocaleDateString()}</span>
                                       <div className="flex gap-2">
                                          <span className={`text-xs px-2 py-1 rounded ${standup.attended ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}>{standup.attended ? "Present" : "Absent"}</span>
                                          <span className={`text-xs px-2 py-1 rounded ${standup.status === "complete" ? "bg-blue-100 text-blue-700" : "bg-orange-100 text-orange-700"}`}>{standup.status}</span>
                                       </div>
                                    </div>
