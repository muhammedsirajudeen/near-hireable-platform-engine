...
export async function GET(request: NextRequest) {
   try {
      ...
      const prds = await PRD.find(query).populate("userId", "name email").sort({ submittedAt: -1 }).lean();
...
export async function PUT(request: NextRequest) {
   try {
      ...
      if (!prdId || !status) {
         return NextResponse.json({ success: false, error: "PRD ID and status are required" }, { status: 400 });
      }
