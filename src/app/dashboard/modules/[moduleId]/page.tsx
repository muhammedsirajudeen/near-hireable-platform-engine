...
import { IModule, ITask, IStandup, ModuleStatus } from "@/models/UserModule";
            ...
            <div className="min-h-screen bg-background text-foreground">
                ...
                <div className="max-w-4xl mx-auto px-4 py-8">
                    <div className="mb-8 flex justify-between items-center">
                        ...
                        <span
                            className={`px-3 py-1 rounded-full text-sm font-medium ${moduleData.status === "locked"
                                ...
                                {moduleData.standups.map((standup, idx) => (
                                    ...
                                    >
                                        <div className="flex justify-between items-start mb-2">
                                            ...
                                            <span
                                                className={`text-xs px-2 py-0.5 rounded ${standup.attended
                                                        ? "bg-green-100 text-green-700"
                                                        : "bg-red-100 text-red-700"
                                                    }`}
                                            >
                                                {standup.attended ? "Attended" : "Missed"}
                                            </span>
                                        </div>
                                        <p className="text-foreground">{standup.review || "No review added."}</p>
                                        <div className="mt-2 text-xs flex items-center gap-2">
                                            <span className="font-semibold text-muted-foreground">Status:</span>
                                            <span className={standup.status === "complete" ? "text-green-600" : "text-yellow-600"}>
                    ...
                    <div className="bg-card border border-border rounded-xl shadow-sm p-6">
                        ...
                        <div className="space-y-4 mb-8">
                            {moduleData.tasks && moduleData.tasks.length > 0 ? (
                                moduleData.tasks.map((task, idx) => (
                                    <div
                                        key={task._id || idx}
                                        className="bg-background border border-border p-4 rounded-lg flex flex-col md:flex-row gap-4 justify-between"
                                    >
                                        ...
                                        <div className="flex md:flex-col items-center justify-center md:items-end min-w-[100px]">
                                            <span
                                                className={`px-3 py-1 rounded-full text-xs font-bold uppercase ${task.status === "approved"
                        ...
                        <form onSubmit={handleAddTask} className="mt-6 border-t border-border pt-6">
                            ...
                            <div className="flex gap-4">
                                ...
                                <button
                                    type="submit"
                                    disabled={submitting || !newTaskContent.trim()}
