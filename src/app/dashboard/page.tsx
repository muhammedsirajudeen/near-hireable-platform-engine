                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 pb-24 md:pb-12">
                    <div className="text-center mb-12">
                        ...
                        <p className="text-muted-foreground max-w-2xl mx-auto">
                            Complete the modules to unlock new opportunities.
                        </p>
                    </div>
                    ...
                    {loading ? (
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
                            {userModules.modules.map((module) => (
                                ...
                                >
                                    <div
                                        className={`relative p-6 rounded-xl border-2 transition-all duration-300 h-full ${module.status === "locked"
                                    ...
                                    >
                                        <div className="flex justify-between items-start mb-4">
                                            <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-lg ${module.status === "locked"
                                        ...
                                        <h3 className={`font-semibold mb-2 ${module.status === "locked" ? "text-muted-foreground" : "text-foreground"}`}>
                                            Module {module.moduleId}
                                        </h3>

                                        <p className="text-sm text-muted-foreground mb-4">
                                            {module.status === "locked" ? "Complete previous module to unlock" :
                                                module.status === "completed" ? "Completed" : "Ready to start"}
                                        </p>

                                        {module.status === "unlocked" && (
                                            <span className="w-full py-2 bg-primary text-primary-foreground rounded-lg font-medium text-sm hover:bg-primary/90 transition-colors block text-center">
                                                Open Module
                                            </span>
                                        )}
                                        {module.status === "completed" && (
                                            <span className="w-full py-2 bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400 rounded-lg font-medium text-sm transition-colors block text-center">
                                                Review Module
                                            </span>
                                        )}
